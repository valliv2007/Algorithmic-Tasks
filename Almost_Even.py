...
Complete the function so that it returns an array of integers representing the parts. If the input number is too small 
to split it into requested amount of parts, the additional parts should have value 0. Ignoring the order of the parts, 
...
def split_integer(num, parts):
    res = [num // parts] * parts
    for i in range(num - sum(res)):
        res[-i - 1] += 1
    return res


def bestsplitInteger(n, pp):
    p, bb = divmod(n, pp)
