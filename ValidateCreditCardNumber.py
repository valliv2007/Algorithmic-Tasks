...
def validate(n):
    check_list = []
    for index, num in enumerate(str(n)[::-1]):
        if index % 2:
            if int(num) >= 5:
                check_list.append(int(num) * 2 - 9)
            else:
                check_list.append(int(num) * 2)
        else:
            check_list.append(int(num))
    return sum(check_list) % 10 == 0


def mynewvalidate(n):
    return sum([int(num) if not i % 2 else int(num) * 2 if int(num) < 5 else int(num) * 2 - 9 for i, num in enumerate(str(n)[::-1])]) % 10 == 0


print(mynewvalidate(1714))
