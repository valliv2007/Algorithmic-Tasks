...
At the beginning of each hour (1-12), it chimes out the hour. Given the current time and a number n, determine the time 
...
If the cuckoo bird chimes at initial_time, include those chimes in the count. If the nth chime is reached on the hour, 
...
and once each at "05:15", "05:30", "05:45". At this point it has chimed 16 times, so the 19th chime occurs 
...
def cuckoo_clock(initial_time, n):
    reminder = int(initial_time[-2:]) % 15 if int(initial_time[-2:]) % 15 else 15
    minutes = int(initial_time[-2:]) - reminder
    hours = int(initial_time[:2]) if minutes >= 0 else int(initial_time[:2]) - 1
    while n > 0:
        minutes += 15
        if minutes == 60 or not minutes:
            minutes = 0
            hours += 1
            if hours == 13:
                hours = 1
